...
install -d -m 755 "$APPS/social-correlator" "$STACK"

sanitize(){ [ -f "$1" ] || return 0; tr '\240' ' ' <"$1" | tr -d '\r' >"$1.__c" && mv -f "$1.__c" "$1"; }

# ===== social-correlator: استخراج حسابات + ربط قواعدي قوي =====
...
D
sanitize "$APPS/social-correlator/Dockerfile"

# ===== compose: إضافة الخدمة =====
APPSY="$STACK/docker-compose.apps.links.yml"
cat >"$APPSY"<<'YML'
name: ffactory
...
log "build+up social correlator"
docker compose --env-file "$ENV" -f "$APPSY" up -d --build
log "endpoints:"
echo "Correlator: http://127.0.0.1:8088/health"
