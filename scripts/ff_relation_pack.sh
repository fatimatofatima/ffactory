...
[ -f "$ENVF" ] || die ".env مفقود"; docker network inspect "$NET" >/dev/null 2>&1 || die "الشبكة $NET مفقودة"
install -d -m 755 "$APPS/relationship-intel" "$STACK"
...
cat >"$APPS/relationship-intel/app.py"<<'PY'
...
# ============ compose ============
EXTY=$STACK/docker-compose.relationship.yml
cat >"$EXTY"<<YML
name: ffactory
...
services:
  relationship-intel:
    ...
    container_name: ffactory_relationship_intel
    env_file: [ ../.env ]
    environment:
      - DB_HOST=db
      - DB_PORT=5432
...
log "[*] build+up relationship-intel"
...
echo "---- health ----"
curl -fsS "http://127.0.0.1:${REL_PORT}/health" || echo "health FAIL on :${REL_PORT}"
