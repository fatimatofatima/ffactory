...
FF="/opt/ffactory"
APPS="$FF/apps"
APP_DIR="$APPS/neural-core"
PROJECT=${COMPOSE_PROJECT_NAME:-ffactory}

[ -d "$APP_DIR" ] || die "مسار التطبيق $APP_DIR غير موجود."

# -----------------------------------------------------
# 1. تحديث متطلبات Python (نماذج Transformers)
# -----------------------------------------------------
log "1/3. تحديث requirements.txt (Transformers لـ AraBERT/MBERT)..."

cat > "$APP_DIR/requirements.txt" << 'REQ_NC'
fastapi>=0.104.0
uvicorn>=0.24.0
# مكتبات النماذج الضخمة (MBERT لـ Tamazight و AraBERT للدارجة/الفصحى)
transformers>=4.35.0
torch>=2.1.0 
sentencepiece # لـ MBERT
REQ_NC

# -----------------------------------------------------
# 2. كتابة كود FastAPI القوي (منطق تحميل النماذج المتعددة)
# -----------------------------------------------------
log "2/3. كتابة كود Neural Core النهائي (دعم اللهجات)..."

cat > "$APP_DIR/main.py" << 'PYTHON_NC'
...
# 3. إعادة بناء الصورة
# -----------------------------------------------------
log "3/3. إعادة بناء صورة Neural Core (لتحميل النماذج الجديدة)..."

# تشغيل البناء
docker compose build --no-cache neural-core || die "🔴 فشل إعادة بناء صورة Neural Core."

log "✅ تم تفعيل Neural Core بالكامل بدعم AraBERT و MBERT للغات المتعددة."
